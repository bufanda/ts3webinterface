...
function create_tree($pid, $place, $alldata, $port, $showicons)
	{
	global $lang;
	global $ts3;
	$gettree='';
	if(!empty($alldata['channel']))
		...
		foreach($alldata['channel'] AS $key=>$value)
			...
			if ($pid==$value['pid'])
				...
				$channelinfo .= "<tr><td style=\'width:150px\' class=\'green1 tooltip\'>".$lang['codec']."</td><td style=\'width:150px\' class=\'green1\'>".($value['channel_codec']==0 ? $lang['codec0']:($value['channel_codec']==1 ? $lang['codec1']:($value['channel_codec']==2 ? $lang['codec2']:($value['channel_codec']==3 ? $lang['codec3']:''))))."</td></tr>";
				...
				if(preg_match("^\[(.*)spacer([\d]+)?\]^", $value['channel_name'], $treffer) AND $value['pid']==0 AND $value['channel_flag_permanent']==1)
					{
					...
					if($chanmaxclient<=$value['total_clients'] AND $value['channel_flag_password']==1)
						{
						$gettree .= "<div class='channel'>".$place."<div class='fullimg'></div><div class='channame' onmouseover=\"return escape('".$channelinfo."')\"><a href=\"javascript:oeffnefenster('site/filebrowser.php?port=".$port."&amp;cid=".$value['cid']."&amp;path=/');\" >".secure($value['channel_name'])."</a>&nbsp;</div><div style=\"float:".$showicons."\">".$chan_img."</div><div class='clear'></div></div>\n";
						}
						...
						{
						$gettree .= "<div class='channel'>".$place."<div class='pwchanimg'></div><div class='channame' onmouseover=\"return escape('".$channelinfo."')\"><a href=\"javascript:oeffnefenster('site/filebrowser.php?port=".$port."&amp;cid=".$value['cid']."&amp;path=/');\" >".secure($value['channel_name'])."</a>&nbsp;</div><div style=\"float:".$showicons."\">".$chan_img."</div><div class='clear'></div></div>\n";
						}
						else
						{
						$gettree .= "<div class='channel'>".$place."<div class='chanimg'></div><div class='channame' onmouseover=\"return escape('".$channelinfo."')\"><a href=\"javascript:oeffnefenster('site/filebrowser.php?port=".$port."&amp;cid=".$value['cid']."&amp;path=/');\" >".secure($value['channel_name'])."</a>&nbsp;</div><div style=\"float:".$showicons."\">".$chan_img."</div><div class='clear'></div></div>\n";
						}
					}
							...
							if($value['cid'] == $u_value['cid'] AND $u_value['client_type']!="1")
										...
										if($cg_value['cgid']==$u_value['client_channel_group_id'])
											...
											if($iconid!=0)
												...
												if($iconid!=100 AND $iconid!=200 AND $iconid!=300 AND $iconid!=500 AND $iconid!=600)
													{
													if(file_exists('icons/'.$_SESSION['server_ip'].'-'.$port.'/icon_'.$iconid))
														{
														$g_img.="<img src=\"site/showfile.php?name=icon_".$iconid."&amp;port=".$port."\" alt=\"\" />";
														}
													}
													else
													{
													$g_img.="<img src=\"site/showfile.php?name=icon_".$iconid."\" alt=\"\" />";
													}
												}
											}
										}
								...
								if($u_value['client_icon_id']!=0)
									...
									if($u_value['client_icon_id']!=100 AND $u_value['client_icon_id']!=200 AND $u_value['client_icon_id']!=300 AND $u_value['client_icon_id']!=500 AND $u_value['client_icon_id']!=600)
										{
										if(file_exists('icons/'.$_SESSION['server_ip'].'-'.$port.'/icon_'.$u_value['client_icon_id']))
											{
											$g_img.="<img src=\"site/showfile.php?name=icon_".$u_value['client_icon_id']."&amp;port=".$port."\" alt=\"\" />";
											}
										}
										else
										{
										$g_img.="<img src=\"site/showfile.php?name=icon_".$u_value['client_icon_id']."\" alt=\"\" />";
										}
									}
								...
								$gettree .= "<div class='client'>".$place."<div class='place'>&nbsp;</div><div class='".$u_status."_img'></div><div class='clientnick'><a onmouseover=\"return escape('".$clientinfo."')\" href=\"javascript:oeffnefenster('site/interactive.php?port=".$port."&amp;clid=".$u_value['clid']."&amp;nick=".str_replace("'", "\'", secure($u_value['client_nickname']))."&amp;action=action');\">".secure($u_value['client_nickname'])."</a>&nbsp;</div><div style=\"float:".$showicons."\">".$g_img."</div>".$u_away_msg."<div style='clear:both'></div></div>\n";
								}
							}
						}
					}
				$gettree .= create_tree($value['cid'], $place."<div class='place'></div>", $alldata, $port, $showicons);
				}
			}
		}
	return $gettree;
