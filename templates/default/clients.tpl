...
<table style="width:100%" class="border" cellpadding="1" cellspacing="0">
	...
	<tr>
		...
		<form method="post" action="index.php?site=clients&amp;port={$port}">
	...
	<tr>
		...
		{if $pages!=0}
			{while $print_pages <= $pages}
				{if $print_pages == 1}
					<a class="paging" href="index.php?site=clients&amp;port={$port}&amp;getstart=0{if isset($smarty.get.sortby)}&amp;sortby={$smarty.get.sortby}{/if}{if isset($smarty.get.sorttype)}&amp;sorttype={$smarty.get.sorttype}{/if}">{$print_pages}</a>	
				{else}
					-<a class="paging" href="index.php?site=clients&amp;port={$port}&amp;getstart={$countstarted}{if isset($smarty.get.sortby)}&amp;sortby={$smarty.get.sortby}{/if}{if isset($smarty.get.sorttype)}&amp;sorttype={$smarty.get.sorttype}{/if}">{$print_pages}</a>
				{/if}
	...
	<tr>
		<td class="thead">
		<a class="headlink" href="index.php?site=clients&amp;port={$port}&amp;sortby=cldbid&amp;sorttype={if $sortby == 'cldbid' AND $sorttype == $smarty.const.SORT_ASC}desc{else}asc{/if}">{$lang['dbid']}</a>
		</td>
		<td class="thead">
		<a class="headlink" href="index.php?site=clients&amp;port={$port}&amp;sortby=unique&amp;sorttype={if $sortby == 'client_unique_identifier' AND $smarty.const.SORT_ASC}desc{else}asc{/if}">{$lang['uniqueid']}</a>
		</td>
		<td class="thead">
		<a class="headlink" href="index.php?site=clients&amp;port={$port}&amp;sortby=name&amp;sorttype={if $sortby == 'client_nickname' AND $sorttype == $smarty.const.SORT_ASC}desc{else}asc{/if}">{$lang['nickname']}</a>
		</td>
		<td class="thead">
		<a class="headlink" href="index.php?site=clients&amp;port={$port}&amp;sortby=created&amp;sorttype={if $sortby == 'client_created' AND $sorttype == $smarty.const.SORT_ASC}desc{else}asc{/if}">{$lang['created']}</a>
		</td>
		<td class="thead">
		<a class="headlink" href="index.php?site=clients&amp;port={$port}&amp;sortby=lastcon&amp;sorttype={if $sortby == 'client_lastconnected' AND $sorttype == $smarty.const.SORT_ASC}desc{else}asc{/if}">{$lang['lastonline']}</a>
		</td>
		<td class="thead">
		<a class="headlink" href="index.php?site=clients&amp;port={$port}&amp;sortby=status&amp;sorttype={if $sortby == 'clid' AND $sorttype == $smarty.const.SORT_ASC}desc{else}asc{/if}">{$lang['status']}</a>
		</td>
	...
	{while $showclients <= $duration AND isset($clientdblist[$getstart])}
		...
		<tr>
			...
			<td class="{$td_col} center">
			<form method="post" action="index.php?site=cleditperm&amp;port={$port}&amp;cldbid={$clientdblist[{$getstart}]['cldbid']}">
